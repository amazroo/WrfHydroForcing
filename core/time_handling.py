...
def find_hourly_nbm_neighbors(supplemental_precip, config_options, d_current, mpi_config):
    """
    Function to calculate the previous and next NBM/CORE/CONUS files.
    :param supplemental_precip:
    :param config_options:
    :param d_current:
    :param mpi_config:
...
def find_hourly_nbmqmd_neighbors(supplemental_precip, config_options, d_current, mpi_config):
    """
    Function to calculate the previous and next NBM/QMD/CONUS files.
    :param supplemental_precip:
    :param config_options:
    :param d_current:
    :param mpi_config:
    ...
    # Set the input file frequency to be 6-hourly for NBM-QMD for all the forecast hours f001:f240.
    supplemental_precip.input_frequency = 360.0
    ...
    # Calculate expected file paths.
    if supplemental_precip.keyValue == 22:   # CONUS
        tmp_file1 = supplemental_precip.inDir + "/blend." + \
            current_nbm_cycle.strftime('%Y%m%d') + \
            "/" + current_nbm_cycle.strftime('%H') + \
    ...
    # Ensure we have the necessary new file
    if mpi_config.rank == 0:
        if not os.path.isfile(supplemental_precip.file_in2) and ((supplemental_precip.keyValue == 22)):
            config_options.statusMsg = "NBM file {} not found, will attempt to use {} instead.".format(
                    supplemental_precip.file_in2, supplemental_precip.file_in1)
            err_handler.log_warning(config_options, mpi_config)
            supplemental_precip.file_in2 = supplemental_precip.file_in1
